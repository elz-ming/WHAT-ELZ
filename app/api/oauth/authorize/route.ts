...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .card { background: white; border: 1px solid #e4e4e7; border-radius: 12px; padding: 2rem; width: 100%; max-width: 400px; }
    ...
    label { display: block; font-size: 0.75rem; font-weight: 500; color: #3f3f46; margin-bottom: 0.5rem; letter-spacing: 0.05em; text-transform: uppercase; font-family: monospace; }
    input[type="password"] { width: 100%; border: 1px solid #e4e4e7; border-radius: 6px; padding: 0.625rem 0.75rem; font-size: 0.875rem; color: #09090b; outline: none; margin-bottom: 1rem; transition: border-color 0.15s; }
    input[type="password"]:focus { border-color: #09090b; }
    button { width: 100%; background: #09090b; color: white; border: none; border-radius: 6px; padding: 0.625rem 1rem; font-size: 0.875rem; font-weight: 500; cursor: pointer; transition: opacity 0.15s; }
...
<body>
  <div class="card">
    ...
    ${p.error ? `<div class="error">Invalid token — please try again.</div>` : ""}
...
export async function GET(req: NextRequest) {
  ...
  return buildHtml({
    ...
    error:                 u.searchParams.has("error"),
  });
}
...
export async function POST(req: NextRequest) {
  ...
  if (!data?.value || token !== data.value) {
    return buildHtml({ redirect_uri, state, code_challenge, code_challenge_method, error: true });
  }
