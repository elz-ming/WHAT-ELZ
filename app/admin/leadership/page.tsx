...
export const metadata: Metadata = { title: 'Leadership — whatelz.ai Admin' };
...
export default async function AdminLeadershipPage() {
  const entries = await listLeadership(false);

  return (
    <div className="max-w-3xl space-y-8">
      <div className="border-b border-zinc-200 pb-6">
        <p className="font-mono text-xs uppercase tracking-widest text-zinc-400">Profile</p>
        <h1 className="mt-2 text-2xl font-semibold tracking-tight text-zinc-900">Leadership</h1>
        <p className="mt-1 text-sm text-zinc-500">
          Edit entries via Claude Chat using <span className="font-mono">update_leadership</span> or <span className="font-mono">patch_leadership_content</span>.
        </p>
      </div>

      <div className="divide-y divide-zinc-100 rounded border border-zinc-200">
        {entries.length === 0 ? (
          <p className="px-4 py-6 text-sm text-zinc-400">No leadership entries yet.</p>
        ) : entries.map(entry => (
          <div key={entry.id} className="px-4 py-4">
            <div className="flex items-start justify-between gap-4">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <p className="text-sm font-semibold text-zinc-900">{entry.organisation}</p>
                ...
                <p className="mt-0.5 font-mono text-xs text-zinc-400">
                  {formatPeriod(entry.start_date, entry.end_date)} · /leadership/{entry.slug}
                </p>
                {entry.tags.length > 0 && (
